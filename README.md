...
## Pipeline comparison
Compared to HyperDrive, many more models were implemented in AutoML. AutoML implemented models like VotingEnseble, StackEnsemble, xgboost, lightgbm, logistic regression etc. whereas only logistic regression was implemented in HyperDrive. Due to this higher performance using AutoML was expected. VotingEnsemble from AutoML gave the best performance with accuracy of 0.9178 compared to 0.9118 of tuned logistic regression from HyperDrive. Performance in AutoML was dominated by ensemble models. Compared to traditional logistic regression model, ensemble models have many more hyperparameters to tune and a fine tuned ensemble model can fit even non-linear pattern in data which results in higher performance. Scope of fine-tuning was comparatively very less in HyperDrive setup.
